...
struct ContentView: View {
    ...
    var dynamicNotchSize: CGSize {
        ...
        let musicHoverInlineActive = vm.notchState == .closed && musicHoverExpansionActive
        if inlineSneakPeekActive || musicHoverInlineActive {
        ...
        if coordinator.currentView == .commandBar {
            let hasResponse = CommandBarCoordinator.shared.viewModel?.conversation.isEmpty == false
            return CGSize(width: baseSize.width, height: hasResponse ? 320 : 110)
        }
    ...
    @State private var isHoveringNextControl: Bool = false
    ...
    @Default(.notchActivationModesSwapped) var notchActivationModesSwapped

    /// True while the command bar is the active tab — used to suspend the
    /// `shouldHideUntilHover` off-screen offset so the bar is always visible
    /// regardless of minimalistic-UI / hide-non-notch settings (fix #16).
    @State private var commandBarIsActive: Bool = false
    ...
    /// Also suspended while the command bar is summoned (fix #16) — otherwise a
    /// ⌘⇧Space press on a non-notch screen with `hideNonNotchUntilHover` on
    /// leaves the bar offset far above the visible region.
    ...
    /// Pill shape for Island mode with animated corner radius transitions.
    private var currentPillShape: MetamorphiaPillShape {
        let radius: CGFloat
        if vm.notchState == .open {
            radius = enableMinimalisticUI
                ? minimalisticCornerRadiusInsets.opened.top
                : islandPillCornerRadiusInsets.opened
        } else {
            // Use half the closed height for a true capsule shape
            radius = max(vm.closedNotchSize.height / 2, islandPillCornerRadiusInsets.closed.standard)
        }
        return MetamorphiaPillShape(cornerRadius: radius)
    }
    ...
    var body: some View {
        ...
        let isIslandMode = self.isIslandMode
        ...
        // Extra top padding to detach pill from screen edge in Island mode
        let pillTopOffset: CGFloat = isIslandMode ? islandTopOffset : 0
        ...
        ZStack(alignment: .top) {
            let mainLayout = NotchLayout()
                ...
                // Extra horizontal inset for Island mode so the shadow
                // is not clipped by the outer frame constraint
                .padding(.horizontal, isIslandMode ? islandShadowInset : 0)
                .padding(.top, pillTopOffset)

            mainLayout
                .conditionalModifier(!useModernCloseAnimation) { view in
                    let hoverAnimation = Animation.bouncy.speed(1.2)
                    let notchStateAnimation = vm.notchTransitionStyle == .commandBarCollapse
                        ? MetamorphiaViewModel.commandBarCollapseAnimation
                        : Animation.spring.speed(1.2)
                    return view
                ...
                .conditionalModifier(useModernCloseAnimation) { view in
                    ...
                    let notchAnimation = vm.notchTransitionStyle == .commandBarCollapse
                        ? MetamorphiaViewModel.commandBarCollapseAnimation
                ...
                .conditionalModifier(interactionsEnabled) { view in
                    view
                        ...
                        // Use a single SimultaneousGesture so the 1-second
                        // long-press recognizer doesn't delay or swallow the
                        // tap. SwiftUI's default chaining of `.onTapGesture`
                        // followed by `.onLongPressGesture` causes the system
                        // to wait for the long-press window before deciding
                        // which one to deliver — that wait is exactly the
                        // "click does nothing" symptom the user was hitting.
                        .gesture(
                            SimultaneousGesture(
                                TapGesture(),
                ...
                .onChange(of: vm.notchState) { _, newState in
                    ...
                    if newState != .closed {
                        ...
                        isHoveringPrevControl = false
                        isHoveringNextControl = false
                        if musicHoverExpansionActive {
                            withAnimation(.smooth(duration: 0.24)) {
                                musicHoverExpansionActive = false
                            }
                        }
                    }
                    if newState == .closed {
                ...
                .onChange(of: coordinator.currentView) { _, newValue in
                    ...
                    // Keep the force-reveal flag (#16) in sync with the
                    // active tab so switching *away* from commandBar lets
                    // `shouldHideUntilHover` resume.
                    commandBarIsActive = (newValue == .commandBar)
                }
                .onReceive(NotificationCenter.default.publisher(for: .commandBarDidSummon)) { _ in
                    commandBarIsActive = true
                }
                .onReceive(NotificationCenter.default.publisher(for: .commandBarDidDismiss)) { _ in
                    commandBarIsActive = false
                }
...
//                    .keyboardShortcut("E", modifiers: .command)
        ...
        .frame(
            maxWidth: dynamicNotchSize.width + (isIslandMode ? islandShadowInset * 2 : 0),
            maxHeight: dynamicNotchSize.height + currentShadowPadding + (isIslandMode ? islandTopOffset : 0),
            alignment: .top
        )
          ...
          VStack(alignment: .leading) {
              VStack(alignment: .leading) {
                  if coordinator.firstLaunch {
                      ...
                      if vm.notchState == .minimized {
                          // Agent run is still running in the background —
                          // render the slim set-aside bar with pulse dot +
                          // live status. Tapping it calls `vm.restore()`.
                          NotchMinimizedView()
                      } else if coordinator.expandingView.type == .battery && coordinator.expandingView.show && vm.notchState == .closed && Defaults[.showPowerStatusNotifications] {
                        HStack(spacing: 0) {
                            HStack {
              ...
              ZStack {
                  // Match the fully opened music mechanism: tab content mounts
                  // only through the open-notch transition, while `currentView`
                  // still selects the target size before `vm.open()` runs.
                  if vm.notchState == .open {
                      Group {
                          switch coordinator.currentView {
                            ...
                            case .commandBar:
                                // ViewModel-nil fallback (#2): don't silently
                                // pretend the user pressed Home — show a
                                // placeholder so they see something landed,
                                // and log. A re-render is driven by
                                // `CommandBarCoordinator.viewModel` didSet
                                // calling `objectWillChange.send()`.
                                if let aiVM = CommandBarCoordinator.shared.viewModel {
                                    NotchCommandBarView(viewModel: aiVM)
                                } else {
                                    commandBarLoadingPlaceholder
                                }
                            case .retrace:
                                NotchRetraceView()
                          }
    ...
    /// not yet been wired (startup race — see fix #2). Shows a minimal
    /// "initializing" shimmer so the user has visual confirmation their
    /// ⌘⇧Space landed, instead of silently reverting to the home tab.
    private var commandBarLoadingPlaceholder: some View {
        HStack(spacing: 10) {
            ProgressView()
                .controlSize(.small)
                .tint(.white.opacity(0.6))
            Text("Starting command bar…")
    ...
    @ViewBuilder
    func MetamorphiaFaceAnimation() -> some View {
        HStack {
            HStack {
                Rectangle()
                    .fill(.clear)
    ...
    private func MusicLiveActivity(secondary preResolvedSecondary: MusicSecondaryLiveActivity? = nil) -> some View {
        ...
        let musicHoverControlsActive = musicHoverExpansionActive && secondary == nil
        ...
        let effectiveCenterWidth = (inlineSneakPeekActive || musicHoverControlsActive) ? 380 : centerBaseWidth
        ...
        let aspectLandscape = musicManager.albumArt.size.width / musicManager.albumArt.size.height > 1.0
        let concentricRadius = ConcentricCorners.closedNotchChildRadius
        let albumArtRadius = aspectLandscape ? concentricRadius / 3.0 : concentricRadius
        HStack(spacing: 0) {
            ...
            Rectangle()
                ...
                .overlay(
                    HStack(alignment: .top) {
                        if musicHoverControlsActive {
                            musicHoverControlIcon(
                                systemName: "backward.fill",
                                isHovering: isHoveringPrevControl,
                                edge: .leading
                            )
                            .onHover { hovering in
                                withAnimation(.smooth(duration: 0.16)) {
                                    isHoveringPrevControl = hovering
                                }
                            }
                            Spacer(minLength: vm.closedNotchSize.width)
                            musicHoverControlIcon(
                                systemName: "forward.fill",
                                isHovering: isHoveringNextControl,
                                edge: .trailing
                            )
                            .onHover { hovering in
                                withAnimation(.smooth(duration: 0.16)) {
                                    isHoveringNextControl = hovering
                                }
                            }
    ...
    @ViewBuilder
    private func musicHoverControlIcon(systemName: String, isHovering: Bool, edge: HorizontalEdge) -> some View {
        let tint = Defaults[.coloredSpectrogram] ? Color(nsColor: musicManager.avgColor) : Color.white
        ZStack {
    ...
    var dragDetector: some View {
        if lockScreenManager.isLocked {
            ...
            Color.clear
                .frame(maxWidth: .infinity, maxHeight: .infinity)
                .contentShape(Rectangle())
                .onDrop(of: [.data], isTargeted: $vm.dragDetectorTargeting) { providers in
    ...
    /// Route a file drop to the command bar when the user holds Option.
    /// Summons the command bar first so the badge has a home, then attaches
    ...
    /// Short-click on the notch → always summon the Command Bar.
    /// We deliberately ignore `notchActivationModesSwapped` here. The swap
    /// toggle (a hidden accessibility feature) had been silently flipped on,
    /// which made every click route to `openNotch()` and showed the music
    /// home view instead of the bar — producing the "click does the wrong
    /// thing" frustration. The setting is still in the Defaults schema for
    ...
    private func handleNotchTap() {
        ...
        // hover-expansion prev/next control must drive the music session
        // directly — opening the Command Bar here would feel wrong and
        // defeat the point of the inline transport.
        if isHoveringPrevControl {
            NSLog("🔔 [Metamorphia] handleNotchTap fired → previousTrack")
            if Defaults[.enableHaptics] { triggerHapticIfAllowed() }
            musicManager.previousTrack()
            return
        }
        if isHoveringNextControl {
            NSLog("🔔 [Metamorphia] handleNotchTap fired → nextTrack")
            if Defaults[.enableHaptics] { triggerHapticIfAllowed() }
            musicManager.nextTrack()
            return
        }
        if isHoveringClosedMusicWaveformControl {
            NSLog("🔔 [Metamorphia] handleNotchTap fired → togglePlay")
            if Defaults[.enableHaptics] { triggerHapticIfAllowed() }
            musicManager.togglePlay()
            return
        }

        NSLog("🔔 [Metamorphia] handleNotchTap fired → toggle Command Bar")
    ...
    /// Long-press on the notch → open the notch tabs. (Always — same
    ...
    /// Mirrors `isClosedMusicGestureContext` but without the
    /// `!coordinator.expandingView.show` gate — hover-to-expand should still
    /// fire while a short sneak peek is auto-playing, not just after it
    /// dismisses.
    private var hasClosedMusicTrack: Bool {
        vm.notchState == .closed
            && coordinator.musicLiveActivityEnabled
            && closedMusicContentEnabled
            && !vm.hideOnClosed
    ...
    private func handleHover(_ hovering: Bool) {
        ...
        if hovering {
            ...
            if isHoveringPrevControl { isHoveringPrevControl = false }
            if isHoveringNextControl { isHoveringNextControl = false }
        }

        updateMusicHoverExpansion(hovering: hovering)
    ...
    /// Expand the closed-notch music row sideways when the mouse enters the
    /// notch and a track is active; collapse it back when the mouse leaves.
    ///
    /// We intentionally skip this when `openNotchOnHover` is enabled — that
    /// mode already opens the full notch on hover, so the inline expansion
    /// would just double up.
    private func updateMusicHoverExpansion(hovering: Bool) {
        if hovering {
            guard !Defaults[.openNotchOnHover],
                  hasClosedMusicTrack else {
                if musicHoverExpansionActive {
                    withAnimation(.smooth(duration: 0.24)) {
                        musicHoverExpansionActive = false
                    }
            ...
            if !musicHoverExpansionActive {
                withAnimation(.smooth(duration: 0.24)) {
                    musicHoverExpansionActive = true
                }
            }
        } else if musicHoverExpansionActive {
            withAnimation(.smooth(duration: 0.24)) {
                musicHoverExpansionActive = false
            }
        }
    ...
    private func shouldPreventAutoClose() -> Bool {
        coordinator.firstLaunch || hasAnyActivePopovers() || vm.isAutoCloseSuppressed || SharingStateManager.shared.preventNotchClose || (Defaults[.terminalStickyMode] && coordinator.currentView == .terminal) || coordinator.currentView == .commandBar
    }
    ...
    private func handleDownGesture(translation: CGFloat, phase: NSEvent.Phase) {
        // Scroll inside the command bar must not collapse / close the bar
