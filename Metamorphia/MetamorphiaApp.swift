...
extension AppDelegate {
    /// Resolves the live AppDelegate. We can't rely solely on
    /// `NSApplication.shared.delegate as? AppDelegate` because under
    /// SwiftUI's `@NSApplicationDelegateAdaptor`, the cast can intermittently
    /// return nil when called from a hotkey handler trampolined back through
    /// a `Task { @MainActor }` boundary (the delegate is technically still
    /// wired but the `delegate` property goes through a SwiftUI proxy that
    /// occasionally races during early lifecycle / re-entrance). We capture
    /// the real instance in `init()` and fall back to the cast only if that
    /// hasn't run yet.
    static var shared: AppDelegate? {
        if let live = liveInstance { return live }
        return NSApplication.shared.delegate as? AppDelegate
    }

    fileprivate static weak var liveInstance: AppDelegate?
}
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    let remoteCommandListener = RemoteCommandListener.shared
...
//    private var windowSizeUpdateWorkItem: DispatchWorkItem?
    ...
    /// Called from `AICommandViewModel.updatePreferredHeight(_:)` after the
    /// command bar's measured content height changes. Piggybacks on the
    /// existing debounced resize so the spring animation reads as one
    /// smooth grow/shrink instead of a rapid-fire stutter.
    func commandBarPreferredHeightDidChange() {
        guard coordinator.currentView == .commandBar,
              vm.notchState == .open else { return }
    ...
    func applicationWillTerminate(_ notification: Notification) {
        let userInfo: [String: Any] = [
            MetamorphiaDistributedNotifications.UserInfoKey.sourcePID: NSNumber(value: ProcessInfo.processInfo.processIdentifier)
        ]
        DistributedNotificationCenter.default().postNotificationName(
            MetamorphiaDistributedNotifications.didBecomeIdle,
            object: nil,
            userInfo: userInfo,
    ...
    /// Force-unhides the notch windows if they were ordered-out under lock
    /// (see `hideWindowsForLock`). Called from `CommandBarCoordinator.summon`
    /// so a ⌘⇧Space press during/after unlock can't find an invisible notch.
    /// `reason` is recorded for diagnostics only.
    func unhideWindowsIfNeeded(reason: String) {
        guard windowsHiddenForLock else { return }
        print("[AppDelegate] unhideWindowsIfNeeded — reason=\(reason)")
    ...
    private func createMetamorphiaWindow(for screen: NSScreen, with viewModel: MetamorphiaViewModel)
        -> NSWindow
    {
    ...
    private func calculateRequiredNotchSize() -> CGSize {
        ...
        if coordinator.currentView == .timer {
            ...
            return CGSize(
                width: vm.closedNotchSize.width + 140,
                height: vm.effectiveClosedNotchHeight
            )
        } else if coordinator.currentView == .commandBar {
            // Dynamic — the command bar reports its measured content
            // height via a PreferenceKey. Clamp tight on the low end so the
            // idle bar hugs its single-line input row, and cap on the high
            // end so a long response can't gobble the screen. The +20pt
            // offset reserves space for the tab header above the bar
            // without padding-out the idle state.
            let cmdVM = CommandBarCoordinator.shared.viewModel
            let preferred = cmdVM?.commandBarPreferredHeight ?? 56
            ...
            if compacted {
                ...
                let screenCap: CGFloat = {
                    ...
                    // Cap at HALF the visible screen so the notch can't
                    // try to take over the desktop on long responses.
                    // Anything taller turns into a wall of text the user
                    // has to scroll INSIDE the notch anyway — better to
                    // bound the outer size and let the transcript scroll.
                    return max(160, (visible - 20) / 2)
                }()
                // Chrome budget: header (~24pt when open) + mainLayout
                // outer .bottom padding (12pt) + shadow inset (18pt) +
                // small safety margin = ~60pt that the inner preference
                // key DOES NOT measure. Without this, the response
                // bubble's bottom rounded corner gets eaten by the
                // notch's bottom curve.
                baseSize.height = max(72, min(screenCap, preferred + 60))
            }
            // Width is locked to the user's configured `openNotchWidth`.
            // Any past widening for long responses caused the SwiftUI
            // content (leading-aligned) to drift LEFT inside an over-wide
            // window, exposing the physical notch. Long paragraphs wrap.
    ...
    /// Adjusts a base notch size for a specific screen by adding Island
    /// shadow insets and top-offset only when the screen lacks a physical notch
    /// and the user has chosen the Island style.
    private func adjustedSizeForScreen(_ baseSize: CGSize, screen: NSScreen) -> CGSize {
        guard shouldUseIslandMode(for: screen.localizedName) else {
            return baseSize
        }
        var adjusted = baseSize
        adjusted.width += islandShadowInset * 2
        adjusted.height += islandTopOffset
        return adjusted
    }
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Register the Services-menu provider so right-click → Services →
        // "Analyze with Metamorphia" on any file in Finder (and the text
        // equivalent on any selection) routes to the agent. Pairs with the
        // NSServices array in Info.plist.
        MetamorphiaServicesProvider.register()

        let userInfo: [String: Any] = [
            MetamorphiaDistributedNotifications.UserInfoKey.sourcePID: NSNumber(value: ProcessInfo.processInfo.processIdentifier)
        ]
        DistributedNotificationCenter.default().postNotificationName(
            MetamorphiaDistributedNotifications.didBecomeActive,
            object: nil,
            userInfo: userInfo,
            deliverImmediately: true
        )
        ...
        remoteCommandListener.startMonitoring()
        ...
        Defaults.publisher(.enableShortcuts, options: []).sink { [weak self] _ in
            Task { @MainActor [weak self] in
                guard let self else { return }
                // Intentionally NOT touching `KeyboardShortcuts.isEnabled`
                // globally. That flag is an all-or-nothing kill-switch, and
                // the Command Bar's ⌘⇧Space is the primary entry point to
                // Metamorphia — it must stay alive even when the user disables
                // feature hotkeys. Gate every *other* shortcut individually
                // below.
        ...
        KeyboardShortcuts.onKeyDown(for: .toggleNotchOpen) { [weak self] in
            ...
            case .open:
                viewModel.close()
            case .minimized:
                // A global "toggle" hotkey should bring a set-aside notch
                // back to the front rather than closing it outright — the
                // user set it aside precisely because they want to keep the
                // run going. Restoring is the obvious inverse action.
                viewModel.restore()
            }
        }

        // Keep the global flag ON — the Command Bar hotkey must work even when
        // the user disables per-feature shortcuts. Per-feature gating happens
        // inside `updateFeatureShortcutAvailability` via per-name enable/disable.
    ...
    private func installTopMenuItemsIfNeeded() {
        ...
        if mainMenu.items.contains(where: { $0.identifier?.rawValue == "Metamorphia.Focus.Menu" }) {
        ...
        focusMenuItem.identifier = NSUserInterfaceItemIdentifier("Metamorphia.Focus.Menu")
        ...
        accessibilityMenuItem.identifier = NSUserInterfaceItemIdentifier("Metamorphia.Accessibility.Menu")
        ...
        permissionsMenuItem.identifier = NSUserInterfaceItemIdentifier("Metamorphia.Permissions.Menu")
    ...
    private func updateFeatureShortcutAvailability() {
        let globalOn = Defaults[.enableShortcuts]
        updateShortcut(.startDemoTimer, isEnabled: globalOn && Defaults[.enableTimerFeature])
        updateShortcut(.clipboardHistoryPanel, isEnabled: globalOn && Defaults[.enableClipboardManager])
        updateShortcut(.colorPickerPanel, isEnabled: globalOn && Defaults[.enableColorPickerFeature])
        updateShortcut(.screenAssistantPanel, isEnabled: globalOn && Defaults[.enableScreenAssistant])
        updateShortcut(.toggleTerminalTab, isEnabled: globalOn && Defaults[.enableTerminalFeature])
        updateShortcut(.toggleSneakPeek, isEnabled: globalOn)
        updateShortcut(.toggleNotchOpen, isEnabled: globalOn)
        // `.commandBar` is intentionally omitted — it must stay usable even
        // when the user disables the global hotkey kill-switch. That's the
        // point of being the primary AI entry point.
        KeyboardShortcuts.enable(.commandBar)
    }
    ...
    @objc func adjustWindowPosition(changeAlpha: Bool = false) {
        if Defaults[.showOnAllDisplays] {
            ...
            if window == nil {
                window = createMetamorphiaWindow(for: selectedScreen, with: vm)
            }
...
final class MediaControlsStateCoordinator {
    ...
    private func cacheAndDisableMusicLiveActivity() {
        if Defaults[.cachedMusicLiveActivityPreference] == nil {
            Defaults[.cachedMusicLiveActivityPreference] = MetamorphiaViewCoordinator.shared.musicLiveActivityEnabled
        }

        if MetamorphiaViewCoordinator.shared.musicLiveActivityEnabled {
            MetamorphiaViewCoordinator.shared.musicLiveActivityEnabled = false
        }
    }
    ...
    private func restoreMusicLiveActivity(clearCache: Bool) {
        ...
        if MetamorphiaViewCoordinator.shared.musicLiveActivityEnabled != cached {
            MetamorphiaViewCoordinator.shared.musicLiveActivityEnabled = cached
        }
