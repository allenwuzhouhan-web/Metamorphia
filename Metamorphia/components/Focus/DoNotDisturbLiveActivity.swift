...
struct DoNotDisturbLiveActivity: View {
    ...
    private enum ToastTiming {
        static let activeDisplay: UInt64 = 2500  // full focus label linger before compacting
    ...
    private var iconWingWidth: CGFloat {
        (manager.isDoNotDisturbActive || isExpanded || showInactiveIcon) ? minimalWingWidth : 0
    }

    private var labelWingWidth: CGFloat {
        guard shouldShowLabel else {
            return compactBalancingWingWidth
        }
    ...
    private var compactBalancingWingWidth: CGFloat {
        if focusToastMode {
            return 0
        }

        return (manager.isDoNotDisturbActive || isExpanded || showInactiveIcon) ? minimalWingWidth : 0
    }
