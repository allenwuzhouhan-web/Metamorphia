            ...
            GeometryReader { geo in
                Group {
                    if progressBarStyle == .segmented {
                        ...
                        SegmentedProgressContent(value: value, geometry: geo, colorMode: colorMode)
                    } else {
        ...
        return Group {
            if colorMode == .volume {
                Capsule()
                    ...
                    .shadow(color: useShadow ? .white : .clear, radius: useShadow ? 8 : 0, x: 3)
            } else if shouldUseColorCoding, let mode = colorMode?.colorCodingMode {
                Capsule()
