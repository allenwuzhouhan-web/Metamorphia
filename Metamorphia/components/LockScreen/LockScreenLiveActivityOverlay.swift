...
struct LockScreenLiveActivityOverlay: View {
	...
	private var horizontalPadding: CGFloat {
		isIslandMode ? 8 : cornerRadiusInsets.closed.bottom
	}
	...
	private var topOffset: CGFloat {
		isIslandMode ? islandTopOffset : 0
	}
	...
	var body: some View {
		...
		.clipShape(
			isIslandMode
				? AnyShape(MetamorphiaPillShape(
					cornerRadius: max(notchSize.height / 2, islandPillCornerRadiusInsets.closed.standard)
				  ))
				: AnyShape(NotchShape(
...
extension LockScreenLiveActivityOverlay {
	static func collapsedScale(for notchSize: CGSize, isIslandMode: Bool = false) -> CGFloat {
		let indicatorSize = max(0, notchSize.height - 12)
		let horizontalPadding = isIslandMode ? 8.0 : cornerRadiusInsets.closed.bottom
