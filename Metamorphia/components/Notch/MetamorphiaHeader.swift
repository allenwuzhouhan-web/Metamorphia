...
struct MetamorphiaHeader: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            HStack(spacing: 4) {
                if vm.notchState == .open && !Defaults[.enableMinimalisticUI] {
                    // Minimize — only visible while an agent run is in
                    // flight, since there's no point setting an idle notch
                    // aside. Collapses to `NotchMinimizedView` without
                    // cancelling the run.
                    if coordinator.currentView == .commandBar,
                       CommandBarCoordinator.shared.viewModel?.isProcessing == true {
                        Button(action: {
                            vm.minimize()
                        }) {
                            Capsule()
                        ...
                        .help("Minimize while work continues")
                    }
...
private extension MetamorphiaHeader {
