...
struct AlbumArtView: View {
    ...
    @ObservedObject var vm: MetamorphiaViewModel
    let albumArtNamespace: Namespace.ID
...
struct NotchHomeView: View {
    ...
    private var mainContent: some View {
        ...
        // Inherit the open spring from `vm.open()` instead of overriding it
        // with `.smooth.speed(0.9)`. The `.move(edge: .top)` was creating a
        // visible double-movement (content slid down while the window was
        // still expanding) — opacity + blur alone reads as a clean morph.
...
private struct MediaOutputPickerButton: View {
    ...
    @EnvironmentObject private var vm: MetamorphiaViewModel
