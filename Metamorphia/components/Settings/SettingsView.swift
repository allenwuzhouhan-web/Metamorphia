...
private enum SettingsTab: String, CaseIterable, Identifiable {
    ...
    var group: SettingsTabGroup {
        ...
        case .aiCommandBar, .aiActivation, .continuum, .designLanguage, .retrace: return .ai
    ...
    var title: String {
        ...
        case .continuum: return String(localized: "Continuum")
        case .designLanguage: return String(localized: "Design Language")
        case .retrace: return String(localized: "Retrace")
    ...
    var systemImage: String {
        ...
        case .aiCommandBar: return "sparkles"
        case .aiActivation: return "hand.tap"
        ...
        case .continuum: return "sparkles"
        case .designLanguage: return "rectangle.on.rectangle.angled"
        case .retrace: return "arrow.counterclockwise.circle"
    ...
    var tint: Color {
        ...
        case .aiCommandBar: return .accentColor
        case .aiActivation: return .accentColor
        ...
        case .continuum: return .accentColor
        case .designLanguage: return .indigo
        case .retrace: return .accentColor
...
struct SettingsView: View {
    ...
    private var settingsSearchIndex: [SettingsSearchEntry] {
        [
            ...
            SettingsSearchEntry(tab: .general, title: "Hide Island during screenshots & recordings", keywords: ["privacy", "screenshot", "recording"], highlightID: SettingsTab.general.highlightID(for: "Hide Island during screenshots & recordings")),
            ...
            // Continuum
            SettingsSearchEntry(tab: .continuum, title: "News", keywords: ["continuum", "news", "kill switch"], highlightID: SettingsTab.continuum.highlightID(for: "News")),
            SettingsSearchEntry(tab: .continuum, title: "Morning brief", keywords: ["morning brief", "continuum"], highlightID: SettingsTab.continuum.highlightID(for: "Morning brief")),
            SettingsSearchEntry(tab: .continuum, title: "Clipboard hints", keywords: ["clipboard hints", "enrichment"], highlightID: SettingsTab.continuum.highlightID(for: "Clipboard hints")),
            SettingsSearchEntry(tab: .continuum, title: "Meeting pre-briefs", keywords: ["meeting briefs", "calendar"], highlightID: SettingsTab.continuum.highlightID(for: "Meeting pre-briefs")),
            SettingsSearchEntry(tab: .continuum, title: "Predictive staging", keywords: ["predictive", "staging", "pre-compute"], highlightID: SettingsTab.continuum.highlightID(for: "Predictive staging")),
            SettingsSearchEntry(tab: .continuum, title: "Attention model", keywords: ["attention model", "learning"], highlightID: SettingsTab.continuum.highlightID(for: "Attention model")),
            SettingsSearchEntry(tab: .continuum, title: "Inspect interest graph", keywords: ["interest graph", "entities", "privacy"], highlightID: SettingsTab.continuum.highlightID(for: "Inspect interest graph")),
            SettingsSearchEntry(tab: .continuum, title: "Forget everything", keywords: ["forget", "reset", "clear data", "privacy", "wipe"], highlightID: SettingsTab.continuum.highlightID(for: "Forget everything")),

            // Design Language
            SettingsSearchEntry(tab: .designLanguage, title: "Learn from uploaded decks", keywords: ["design language", "powerpoint", "deck", "style"], highlightID: SettingsTab.designLanguage.highlightID(for: "Learn from uploaded decks")),
            SettingsSearchEntry(tab: .designLanguage, title: "Use my design language in PowerPoint", keywords: ["powerpoint", "presentation", "style"], highlightID: SettingsTab.designLanguage.highlightID(for: "Use my design language in PowerPoint")),
            SettingsSearchEntry(tab: .designLanguage, title: "Ask before model analysis", keywords: ["privacy", "model", "analysis", "consent"], highlightID: SettingsTab.designLanguage.highlightID(for: "Ask before model analysis")),
            SettingsSearchEntry(tab: .designLanguage, title: "Upload PowerPoint deck", keywords: ["pptx", "ppt", "reference deck"], highlightID: SettingsTab.designLanguage.highlightID(for: "Upload PowerPoint deck")),
    ...
    private func detailView(for tab: SettingsTab) -> some View {
        ...
        case .aiCommandBar:
            SettingsForm(tab: .aiCommandBar) {
                AICommandBarSettings()
            }
        case .aiActivation:
            SettingsForm(tab: .aiActivation) {
                NotchActivationSettings()
            }
        case .liveActivities:
        ...
        case .continuum:
            SettingsForm(tab: .continuum) {
                ContinuumSettingsView()
            }
        case .designLanguage:
            SettingsForm(tab: .designLanguage) {
                DesignLanguageSettingsView()
            }
        case .retrace:
            SettingsForm(tab: .retrace) {
                RetraceSettingsView()
            }
        case .about:
    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                Defaults.Toggle(key: .hideIslandFromScreenCapture) {
                    Text("Hide Island during screenshots & recordings")
                }
                .settingsHighlight(id: highlightID("Hide Island during screenshots & recordings"))
            } header: {
    ...
    var body: some View {
        SwiftUI.Form {
            Section {
                ...
                Text("Monitor your Downloads folder for Chromium-style downloads (.crdownload files) and show a live activity in the Island while downloads are in progress.")
            }
        }
    ...
    private var providerStatusDescription: String {
        ...
        case .betterDisplay:
            if !betterDisplayManager.isDetected {
                return "Install [BetterDisplay](https://betterdisplay.pro) to control external display brightness (and optional volume) through Metamorphia's HUD."
            }
            ...
            return "BetterDisplay OSD events will be routed through Metamorphia's active HUD style. Brightness is always routed; volume is routed when external volume control listener is enabled below. Make sure BetterDisplay's OSD integration is enabled in Settings › Application › Integration."
        case .lunar:
            if !lunarManager.isDetected {
                return "Install [Lunar](https://lunar.fyi) to control external display brightness, contrast, and optional volume through Metamorphia's HUD via DDC."
            }
            ...
            if lunarManager.isConnected {
                return "Connected to Lunar's DDC socket. Brightness and contrast adjustments are shown through Metamorphia's HUD; volume follows when external volume control listener is enabled below."
            }
            ...
            Section {
                ...
                if enableThirdPartyDDCIntegration {
                    ...
                    Text(
                        ...
                        ? "Metamorphia's built-in volume key interception is disabled while external volume listening is on. Volume HUD/OSD will follow \(thirdPartyDDCProvider.displayName) payloads."
                        : "Metamorphia keeps native volume key interception. External provider volume payloads are ignored while this is off."
                    )
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    ...
                    Text("Enable to route BetterDisplay or Lunar display adjustments through Metamorphia's active HUD style.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            } footer: {
                if enableThirdPartyDDCIntegration {
                    Text("Metamorphia always listens to selected-provider brightness events, and listens to provider volume events only when external volume listener is enabled.")
                        .foregroundStyle(.secondary)
                        .font(.caption)
                }
            }
        ...
        Form {
            ...
            Section {
                ...
                if progressBarStyle == .segmented {
                    Text("Color-coded fills are unavailable in Segmented mode. Switch to Hierarchical or Gradient inside Controls › Island to adjust advanced options.")
                        .foregroundStyle(.secondary)
                        .font(.caption)
                } else if Defaults[.useSmoothColorGradient] {
                    Text("Smooth transitions blend Green (0–60%), Yellow (60–85%), and Red (85–100%) through the entire fill. Adjust gradient behavior from Controls › Island.")
                        .foregroundStyle(.secondary)
                        .font(.caption)
                } else {
    ...
    var body: some View {
        Form {
            if !hasAccessibilityPermission && !enableThirdPartyDDCIntegration {
                Section {
                    SettingsPermissionCallout(
                        message: "Accessibility permission lets Island replace the native volume, brightness, and keyboard HUDs.",
            ...
            Section {
                ...
                Text("Requires Accessibility permission so Island can intercept the hardware volume keys.")
                    .foregroundStyle(.secondary)
                    .font(.caption)
            }

            Section {
                if !colorCodingDisabled && Defaults[.useColorCodedBatteryDisplay] {
                ...
                Text("Island Progress Bars")
            } footer: {
                if colorCodingDisabled {
                    Text("Smooth battery color transitions are unavailable in Segmented mode. Switch to Hierarchical or Gradient to adjust this option.")
                        .foregroundStyle(.secondary)
                        .font(.caption)
                } else if Defaults[.useSmoothColorGradient] {
                    Text("Smooth color transitions apply to color-coded battery indicators.")
                        .foregroundStyle(.secondary)
                        .font(.caption)
                } else {
                    Text("Volume progress bars stay white at every level.")
                        .foregroundStyle(.secondary)
                        .font(.caption)
                }
            }
        ...
        Form {
            ...
            Section {
                Defaults.Toggle(key: .showStandardMediaControls) {
                    Text("Show media controls in Island")
                }
                .disabled(enableMinimalisticUI)
                .settingsHighlight(id: highlightID("Show media controls in Island"))
        ...
        Form {
            if !calendarManager.hasCalendarAccess || !calendarManager.hasReminderAccess {
                ...
                Section(
                    ...
                    footer: Text("Displays your next upcoming calendar event above or below the weather capsule. Calendar selection here is independent from the Island calendar filter.")
                        .foregroundStyle(.secondary)
                        .font(.caption)
                ) {
...
struct About: View {
    ...
    var body: some View {
        VStack {
            ...
            VStack(spacing: 0) {
                Divider()
                Text("Made with ❤️ by Johann Endersmith")
                    .foregroundStyle(.secondary)
                    .padding(.top, 5)
                    .padding(.bottom, 7)
                    .multilineTextAlignment(.center)
            ...
            Section {
                ...
                .actionBar {
                    HStack(spacing: 5) {
                        ...
                        Divider()
                        Button {
                            guard let visualizer = selectedListVisualizer,
                                  let removeAt = customVisualizers.firstIndex(of: visualizer) else { return }
                            selectedListVisualizer = nil
                            customVisualizers.remove(at: removeAt)
                ...
                .sheet(isPresented: $isPresented) {
                    VStack(alignment: .leading) {
                        ...
                        HStack {
                            ...
                            Button {
                                guard let parsedURL = URL(string: url), !url.isEmpty else { return }
                            ...
                            .disabled(URL(string: url) == nil || url.isEmpty)
                        }
                    }
    ...
    var body: some View {
        Form {
            Section {
                ...
                Text("Controls whether Island mirrors lock/unlock events with its own live activity and audible chimes.")
            }
            ...
            Section {
                ...
                if !showStandardMediaControls {
                    Text("Enable Island media controls to manage the lock screen panel.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .frame(maxWidth: .infinity, alignment: .leading)
                }
            ...
            Section {
                ...
                Text("Displays your next upcoming calendar event above or below the weather capsule. Calendar selection here is independent from the Island calendar filter.")
            }
...
private func copyLatestCrashReport() {
    ...
    do {
        ...
        let crashFiles = files.filter { $0.contains("Metamorphia") && $0.hasSuffix(".crash") }

        guard let latestCrash = crashFiles.sorted(by: >).first else {
            let alert = NSAlert()
            alert.messageText = "No Crash Reports Found"
            alert.informativeText = "No crash reports found for Metamorphia"
        ...
        Form {
            ...
            if enableShortcuts {
                ...
                Section {
                    ...
                    Text("Toggle the Island open or closed from anywhere.")
                        .multilineTextAlignment(.trailing)
                        .foregroundStyle(.secondary)
                        .font(.caption)
                }
...
private struct TimerPresetListView: View {
    ...
    var body: some View {
        // Iterate by stable UUID, not by index. ForEach over indices crashes
        // SwiftUI's NSOutlineView hover-row coordinator while scrolling whenever
        // the array is mutated (move/delete buttons inside the row), because the
        // coordinator dereferences a row index that has just shifted.
            ...
            if enableStatsFeature {
                ...
                Section {
                    ...
                    if enabledGraphsCount >= 4 {
                        Text("With \(enabledGraphsCount) graphs enabled, the Island will expand horizontally to accommodate all graphs in a single row.")
                            .multilineTextAlignment(.trailing)
                            .foregroundStyle(.secondary)
                            .font(.caption)
                    } else {
