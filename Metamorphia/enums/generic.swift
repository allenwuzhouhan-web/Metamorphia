...
/// Controls how Metamorphia renders on external and non-notched displays.
/// - `notch`: Standard notch shape (concave top corners blending into the screen edge).
/// - `island`: Pill-shaped island with continuously rounded corners,
...
enum ExternalDisplayStyle: String, CaseIterable, Defaults.Serializable, Identifiable {
    ...
    var localizedName: String {
        ...
        case .island:
            return String(localized: "Island")
        }
    }
...
public enum NotchState {
    case closed
    case open
    /// Idle-height bar with a pulsing dot and live status label. The agent
    /// run continues in the background — the notch is just set aside so the
    /// user can keep working. Tapping the minimized bar restores `.open`.
    case minimized
}
