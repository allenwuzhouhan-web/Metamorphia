...
private struct ScrollMonitor: NSViewRepresentable {
    ...
    @MainActor final class Coordinator: NSObject {
        ...
        private func handleScroll(_ event: NSEvent) {
            ...
            guard let isInsideNestedScrollView = scrollHitContext(using: event) else { return }
        ...
        private func scrollHitContext(using event: NSEvent) -> Bool? {
            guard let view = observedView, let window = view.window else { return nil }
            ...
            guard hitArea.contains(localPoint) else { return nil }

            return isNestedScrollViewHit(in: view, at: localPoint)
        }

        private func isNestedScrollViewHit(in view: NSView, at localPoint: NSPoint) -> Bool {
            guard let hitView = view.hitTest(localPoint) else { return false }
