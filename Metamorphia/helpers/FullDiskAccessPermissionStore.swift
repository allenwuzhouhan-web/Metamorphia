...
enum ShelfFolderAccessAuthorization {
    ...
    static func requestAccessProbe(skipDocuments: Bool = false, skipDownloads: Bool = false) {
        if !skipDocuments, let documentsDirectoryURL {
            _ = try? FileManager.default.contentsOfDirectory(at: documentsDirectoryURL, includingPropertiesForKeys: nil)
        }

        if !skipDownloads, let downloadsDirectoryURL {
...
final class FullDiskAccessPermissionStore: ObservableObject {
    ...
    func requestAccessPrompt() {
        ...
        alert.informativeText = "Metamorphia needs Full Disk Access to detect custom Focus indicators and power the Shelf. Click Continue to open Full Disk Access settings, then press the + button and select Metamorphia (we'll reveal it in Finder for you)."
        ...
        alert.icon = Self.permissionIcon
    ...
    private static let permissionIcon: NSImage = {
...
final class ShelfFolderAccessPermissionStore: ObservableObject {
    ...
    func requestAccessPrompt() {
        let hasDocuments = ShelfFolderAccessAuthorization.hasDocumentsAccess()
        let hasDownloads = ShelfFolderAccessAuthorization.hasDownloadsAccess()
        updateStatus(documents: hasDocuments, downloads: hasDownloads)

        if hasDocuments && hasDownloads {
            return
        }

        ShelfFolderAccessAuthorization.requestAccessProbe(
            skipDocuments: hasDocuments,
            skipDownloads: hasDownloads
        )
        beginPollingForStatusChanges()
    }
