...
class CalendarManager: ObservableObject {
    ...
    /// Read-only refresh of Calendar + Reminder authorization status. Never triggers a
    /// system permission dialog. Call from transient surfaces (lock screen, HUD) that
    /// appear on every wake — the first-time grant happens in onboarding or Settings.
    func refreshAuthorizationState() async {
        ...
        if isAuthorized(eventStatus) || isAuthorized(reminderStatus) {
            await reloadCalendarAndReminderLists()
        }
    }
