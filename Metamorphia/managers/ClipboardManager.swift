...
class ClipboardManager: ObservableObject {
    ...
    // Background queue so per-change pasteboard reads, image hashing and the
    // dedup scan never block the main thread / UI.
    private let processingQueue = DispatchQueue(label: "com.metamorphia.clipboard.processing", qos: .utility)
    // A copy of the history owned exclusively by `processingQueue`, used for the
    // off-main dedup pre-check. Reading the @Published `clipboardHistory` from the
    // background queue would race the main-thread mutations in `addToHistory`; this
    // queue-local mirror never does. It is refreshed from the authoritative history
    // after every mutation. Staleness is harmless — `addToHistory` re-dedups on main.
    private var dedupSnapshot: [ClipboardItem] = []

    // Cap on the number of pinned items we persist so the pinned list can't grow unbounded.
    private static let maxPinnedItems = 50

    // Pasteboard UTIs marking content that must not be recorded into history.
    private static let concealedType = NSPasteboard.PasteboardType("org.nspasteboard.ConcealedType")
    private static let transientType = NSPasteboard.PasteboardType("org.nspasteboard.TransientType")
    // Autotype hint some password managers set to suppress clipboard tooling.
    ...
    private init() {
        ...
        cleanupOldFiles()
        // Seed the queue-owned dedup snapshot from the loaded history.
        let snapshot = clipboardHistory
        processingQueue.async { self.dedupSnapshot = snapshot }
    }
    ...
    func startMonitoring() {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 0.5, repeats: true) { [weak self] _ in
            ...
            // Hand the whole check (change-count read, image reads, hashing, dedup
            // scan) to the serial background queue so nothing blocks the main thread.
    ...
    func pinItem(_ item: ClipboardItem) {
        ...
        // Cap pinned items so the pinned list can't grow without bound (history is
        // already capped; pinned previously was not). Drop the oldest, cleaning up
        // any associated image files.
        if pinnedItems.count > ClipboardManager.maxPinnedItems {
            let overflow = Array(pinnedItems.dropLast(ClipboardManager.maxPinnedItems))
            for oldItem in overflow {
                if let fileName = oldItem.imageFileName {
                    let fileURL = ClipboardManager.clipboardDataDirectory.appendingPathComponent(fileName)
                    try? FileManager.default.removeItem(at: fileURL)
                }
            }
            pinnedItems = Array(pinnedItems.suffix(ClipboardManager.maxPinnedItems))
        }
    ...
    // Runs on `processingQueue` (serial), so the lastChangeCount check below is
    // race-free: two rapid copies are processed one at a time and can't interleave.
    private func checkClipboard() {
        ...
        // SECURITY: never record content the source marked transient/concealed
        ...
        // @Published array) so this off-main read can't race main-thread mutations.
        if !dedupSnapshot.contains(where: { isSameContent($0, clipboardItem) }) {
            addToHistory(clipboardItem)
        }
    }

    // True when the current pasteboard item is marked transient/concealed/auto-generated
    // and therefore must not be stored in clipboard history.
    private func shouldSkipPasteboard(_ pasteboard: NSPasteboard) -> Bool {
        guard let types = pasteboard.types else { return false }
        return types.contains(ClipboardManager.transientType)
            || types.contains(ClipboardManager.concealedType)
            || types.contains(ClipboardManager.autoGeneratedType)
    }
    ...
    private func addToHistory(_ item: ClipboardItem) {
        DispatchQueue.main.async { [weak self] in
            ...
            // Refresh the processing-queue-owned dedup snapshot from the
            // authoritative history (value-type copy crossed onto the queue).
            let snapshot = self.clipboardHistory
            self.processingQueue.async { self.dedupSnapshot = snapshot }
        }
    }
