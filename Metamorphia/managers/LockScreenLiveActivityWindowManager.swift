...
class LockScreenLiveActivityWindowManager {
    ...
    private weak var viewModel: MetamorphiaViewModel?
    private var screenChangeObserver: NSObjectProtocol?
    private var workspaceObservers: [NSObjectProtocol] = []
    private var currentNotchSize: CGSize?

    /// Whether the target screen uses Island (pill) mode.
    private var isIslandMode: Bool {
        isIslandModeForScreen(lockContext()?.screen)
    }

    private func isIslandModeForScreen(_ screen: NSScreen?) -> Bool {
        guard let screen else { return false }
        return shouldUseIslandMode(for: screen.localizedName)
    }
    ...
    private func windowSize(for notchSize: CGSize) -> CGSize {
        ...
        let horizontalPadding = isIslandMode ? 8 : cornerRadiusInsets.closed.bottom
        let topOffset: CGFloat = isIslandMode ? islandTopOffset : 0
    ...
    private func handleScreenGeometryChange(reason: String) {
        ...
        if let hostingView {
            ...
            hostingView.rootView = LockScreenLiveActivityOverlay(model: overlayModel, animator: overlayAnimator, notchSize: context.notchSize, isIslandMode: isIslandMode)
        }
    ...
    private func present(notchSize: CGSize, on screen: NSScreen) {
        ...
        let overlayView = LockScreenLiveActivityOverlay(model: overlayModel, animator: overlayAnimator, notchSize: notchSize, isIslandMode: isIslandMode)
    ...
    func showLocked() {
        ...
        let collapsedScale = LockScreenLiveActivityOverlay.collapsedScale(for: context.notchSize, isIslandMode: isIslandMode)
    ...
    private func hideWithAnimation() {
        ...
        if let notchSize = currentNotchSize {
            targetScale = LockScreenLiveActivityOverlay.collapsedScale(for: notchSize, isIslandMode: isIslandMode)
        } else {
            targetScale = 0.7
        }
    ...
    func configure(viewModel: MetamorphiaViewModel) {
