...
final class MemoryUsageMonitor {
    ...
    private func presentRestartAlert(currentUsage: UInt64) {
        ...
        alert.messageText = "Metamorphia memory usage is high"
        alert.informativeText = "The app is using \(formatMegabytes(currentUsage)) MB, above the \(formatMegabytes(thresholdBytes)) MB safe limit. Restart to free memory?"
        // Pre-load the icon synchronously. NSAlert's default caution icon goes
        // through _NSAsynchronousPreparation, and an off-main accessibility
        // query (VoiceOver, screen-reader bridges, our own AX observer) calls
        // `preparedResult` from a worker queue, hits the main-thread assert,
        // and aborts the app.
        alert.icon = Self.cautionIcon
        // Default button is Keep Running so an accidental Return keystroke
        // (e.g. while the user is submitting a command) doesn't silently
        // relaunch the app out from under them.
    ...
    private func relaunchApplication() {
        ...
        workspace.openApplication(at: appURL, configuration: configuration) { launchedApp, error in
            if let error {
                Logger.log("[MemoryMonitor] Failed to launch replacement app: \(error.localizedDescription). Aborting restart.", category: .error)
                return
            }
            guard launchedApp != nil else {
                Logger.log("[MemoryMonitor] Replacement app did not report a running instance. Aborting restart.", category: .error)
                return
            }
            Task { @MainActor in
    ...
    private static let cautionIcon: NSImage = {
        let image = NSImage(systemSymbolName: "exclamationmark.triangle.fill",
            ...
            ?? NSImage()
        // Force the bitmap representation to materialize on the main thread now,
        // so AppKit doesn't reach into _NSAsynchronousPreparation later.
        _ = image.size
        return image
    }()
}
