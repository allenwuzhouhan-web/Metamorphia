...
final class ReminderLiveActivityManager: ObservableObject {
    ...
    private let logger: os.Logger = os.Logger(subsystem: "com.johannendersmith.metamorphia", category: "ReminderLiveActivity")
    ...
    func evaluateCurrentState(at date: Date, source: String = "unknown") async {
        ...
        if entry.triggerDate <= date {
            ...
            if activeReminder != entry {
                ...
                MetamorphiaViewCoordinator.shared.toggleSneakPeek(
                    status: true,
                    type: .reminder,
