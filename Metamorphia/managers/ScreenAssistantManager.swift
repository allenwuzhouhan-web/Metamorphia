...
// Chat message model
struct ScreenAssistantChatMessage: Identifiable, Codable {
    let id = UUID()
    let content: String
    let isFromUser: Bool
    let timestamp: Date
...
class ScreenAssistantManager: NSObject, ObservableObject {
    ...
    @Published var chatMessages: [ScreenAssistantChatMessage] = []
    ...
    private func sendToGeminiAPI(message: String, files: [ScreenAssistantFile]) {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            DispatchQueue.main.async { [weak self] in
                guard let self = self else { return }
                // Pass the key via the x-goog-api-key header so it never appears in the request URL.
                self.performAPIRequest(url: url, requestBody: requestBody, provider: .gemini, googleAPIKey: apiKey)
            }
        }
    }
