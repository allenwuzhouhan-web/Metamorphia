...
struct CustomAppIcon: Codable, Hashable, Equatable, Defaults.Serializable, Identifiable {
    ...
    static let iconDirectory: URL = {
        ...
        let dir = base.appendingPathComponent("Metamorphia", isDirectory: true)
...
enum ClipboardDisplayMode: String, CaseIterable, Codable, Defaults.Serializable {
    ...
    case separateTab = "separateTab" // Separate tab in Island
    ...
    var description: String {
        ...
        case .separateTab: return "Shows copied items in a separate tab within the Island (merges with Notes if enabled)"
        }
    }
}
...
enum ProgressBarStyle: String, CaseIterable, Identifiable, Defaults.Serializable {
    ...
    // Deprecated: renders identically to .hierarchical. Kept for storage compatibility with existing user preferences.
...
extension Defaults.Keys {
    ...
    static let hideIslandFromScreenCapture = Key<Bool>("hideIslandFromScreenCapture", default: false)
    ...
    static let useSmoothColorGradient = Key<Bool>("useSmoothColorGradient", default: false)
    ...
    // Retained for user-defaults migration only — the floating pause/stop squircle
    // was retired in favour of the minimal Apple-style notch layout.
